...
- **Cross-doc drift.** `coop` keeps `docs/ARCHITECTURE.md`, `docs/trust-model.md`, `docs/code-style.md`, `docs/commands.md`, `docs/configuration.md`, `config.example.toml`, and AGENTS.md pinned to the code. If the diff changes a module boundary, trust boundary, command, or config surface, flag the doc that now describes the old shape.
...
- **Missing doc-comments** on previously-undocumented items — that belongs to the conventions agent (AGENTS.md wants Google-style doc-comments on non-trivial public APIs).
